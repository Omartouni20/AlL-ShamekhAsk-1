...
interface AdminLoginProps {
  onLogin: (user: { id: string; name: string; role: 'admin' }) => void;
}

export function AdminLogin({ onLogin }: AdminLoginProps) {
  const [email, setEmail] = useState(''); // هنستخدمه كـ username للحفاظ على نفس UI
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onLogin({ id: data.user.id, name: data.user.name, role: 'admin' });
    } catch (err: any) {
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-[#1e3a8a] to-[#2563eb] px-8 py-10 text-white text-center">
            <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <Shield className="w-8 h-8" />
            </div>
            <h2 className="text-2xl font-bold mb-2">لوحة تحكم الإدارة</h2>
            <p className="text-blue-100 text-sm">سجل دخول للوصول إلى لوحة التحكم</p>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pr-11 pl-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  required
                  dir="ltr"
                />
              </div>
